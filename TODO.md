...
- extend the current local Figure 5/6/7 rule driver beyond the acyclic scalar subset
- broaden rule-driver handling beyond the current `Assign` / `Assume` / branch subset to more lowered instructions
- connect memory and call effects to rule-level `Pre` / `Post` candidates
- extend rule-witness replay beyond the current local scalar query slice
