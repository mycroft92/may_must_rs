...
//! shows exactly which rule changed which node summary, in order.
//!
...
use crate::cfg::{AbstractCfg, AbstractEdge, CfgEdgeId, CfgNodeId, TransferEffect};
use crate::formula::{Formula, SmtModel};
use crate::smt::oracle::{Feasibility, Oracle, OracleError, Validity};
...
impl<'a> RuleEngine<'a> {
    ...
    /// Runs interleaved forward MAY and backward NOT-MAY passes to a fixpoint.
    ///
    /// Each iteration:
    ...
    ///
    /// Sound BugFound verdicts on acyclic CFGs require an under-approximate
    /// witness.  For cyclic CFGs the SMASH orchestrator (`smash::run_smash`)
    /// defers bug-finding to the DART forward-MUST path.
    ///
    ...
    /// (no bug reported); the SMASH orchestrator tries DART instead.
    pub fn bugfound(
        &self,
        entry: CfgNodeId,
        oracle: &Oracle,
        ...
        if !cfg_is_acyclic {
            log::debug!(
                ...
                "[bugfound] entry {:?}: cfg is cyclic — skipping unsound reach∧state \
                 heuristic (deferring bug-finding to DART)",
                entry,
            );
            return Ok(None);
        }
...
pub fn callee_of(node: &crate::cfg::AbstractNode) -> Option<String> {
    node.transfer
        .effects
        .iter()
...
mod tests {
    use super::*;
    use crate::cfg::{TransferEffect, TransferFn};
