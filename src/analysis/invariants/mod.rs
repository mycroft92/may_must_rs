...
use crate::cfg::{AbstractCfg, AssignValue, CfgNodeId, TransferEffect};
use crate::formula::{Formula, Memory, ModelValue, SmtModel, Sort, Term, Var};
...
fn collect_term_vocab(
    ...
    match term {
        ...
        Term::Select(arr, idx) => {
            ...
            if matches!(arr.as_ref(), crate::formula::Memory::Var(_)) {
...
fn run_tier(
    ...
    use crate::analysis::loops::normalize_candidate;
    for candidate in candidates {
        ...
        if crate::analysis::backward::is_tautology(&normalized) {
            continue;
        }
        let result =
        ...
        log::debug!(
            ...
            crate::analysis::backward::pretty_formula(&normalized),
            match &result {
