...
use crate::analysis::interproc::summaries::{MaySummary, NotMaySummary, ProcedureName};
use crate::cfg::substitute_var_in_formula;
use crate::formula::{Formula, Memory, SmtModel, Term, Var};
use crate::smt::oracle::{Oracle, OracleError, Validity};
...
pub struct ContextualSummaryTable {
    ...
    pub loop_invariants: BTreeMap<ProcedureName, Vec<(crate::cfg::CfgNodeId, Formula)>>,
}
...
impl ContextualSummaryTable {
    ...
    pub fn set_loop_invariants(
        &mut self,
        procedure: impl Into<ProcedureName>,
        invariants: Vec<(crate::cfg::CfgNodeId, Formula)>,
    ) {
        self.loop_invariants.insert(procedure.into(), invariants);
    }

    /// Returns loop invariants for `procedure`, or empty if none.
    pub fn get_loop_invariants(&self, procedure: &str) -> &[(crate::cfg::CfgNodeId, Formula)] {
...
fn guess_sort_of(term: &Term) -> crate::formula::Sort {
    match term {
        Term::Real(_) => crate::formula::Sort::Real,
        // BoolToInt produces an Int; everything else we use is Int-sorted.
        _ => crate::formula::Sort::Int,
    }
}
