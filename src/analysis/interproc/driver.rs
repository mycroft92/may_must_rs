...
use crate::analysis::interproc::providers::{CandidateProvider, NoProvider};
use crate::analysis::interproc::scheduler;
use crate::analysis::interproc::smash;
use crate::analysis::interproc::summaries::{MaySummary, NotMaySummary, SummaryTables};
use crate::cfg::adapter::{
...
use crate::cfg::{AbstractCfg, SourceLocation};
use crate::formula::{Formula, Memory, Term, Var};
use crate::frontend::program_graph::FunctionGraph;
use crate::pointer_analysis::andersen::run_alias_analysis;
...
pub fn analyze_module_with_provider(
    ...
    for summary in summaries.summaries().values() {
        summary_tables.add_forward_may(
            ...
            MaySummary {
                precondition: crate::formula::Formula::True,
        ...
        summary_tables.add_notmay(
            ...
            NotMaySummary {
                precondition: crate::formula::Formula::True,
                postcondition: crate::formula::Formula::not(summary.relation.clone()),
            },
        );
    }
    ...
    let mut proc_query_ids: BTreeMap<String, Vec<crate::analysis::interproc::query::QueryId>> =
        BTreeMap::new();
    ...
    for graph in graphs {
        ...
        let interface = crate::analysis::interproc::query::ProcedureInterface::new(
            adapted.name.clone(),
        ...
        for site in &adapted.assertions {
            ...
            let query = crate::analysis::interproc::query::Query::new(
                adapted.name.clone(),
                Formula::True,
                pre_at_assertion,
            );
...
mod tests {
    use super::*;
    use crate::analysis::interproc::providers::ManualProvider;
    use crate::cfg::adapter::ReturnSummary;
