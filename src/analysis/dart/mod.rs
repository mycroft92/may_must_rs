...
use crate::analysis::backward::AssertionResult;
use crate::cfg::adapter::AssertionSite;
use crate::cfg::{AbstractCfg, AssignValue, CfgEdgeId, CfgNodeId, TransferEffect};
use crate::formula::{Formula, Memory, SmtModel, Term, Var};
use crate::smt::oracle::{Feasibility, Oracle};
...
fn subst_formula_walk(
    ...
    for (orig, fresh) in current_version {
        ...
        let int_target = Var::new(orig.clone(), crate::formula::Sort::Int);
        let int_replacement = Term::Var(Var::new(fresh.clone(), crate::formula::Sort::Int));
        f = crate::cfg::substitute_var_in_formula(&int_target, &int_replacement, &f);
        let bool_target = Var::new(orig.clone(), crate::formula::Sort::Bool);
        let bool_replacement = Formula::Var(Var::new(fresh.clone(), crate::formula::Sort::Bool));
        f = crate::cfg::substitute_bool_var_in_formula(&bool_target, &bool_replacement, &f);
    }
    for (region, mem) in memory_state {
        f = crate::cfg::substitute_memory_var_in_formula(region, mem, &f);
    }
    f
}
...
fn subst_term_walk(
    ...
    for (orig, fresh) in current_version {
        let int_target = Var::new(orig.clone(), crate::formula::Sort::Int);
        let int_replacement = Term::Var(Var::new(fresh.clone(), crate::formula::Sort::Int));
        t = crate::cfg::substitute_var_in_term(&int_target, &int_replacement, &t);
    }
    for (region, mem) in memory_state {
        t = crate::cfg::substitute_memory_var_in_term(region, mem, &t);
    }
    t
}
...
mod tests {
    use super::*;
    use crate::cfg::{AssignValue, TransferEffect, TransferFn};
    ...
    fn dart_finds_bug_on_straightline() {
        ...
        let site = AssertionSite {
            id: 0,
            node: assert_node,
            source_location: crate::cfg::SourceLocation::new("", 0, 0),
