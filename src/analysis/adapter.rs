...
fn local_name(function_name: &str, instruction: Instruction) -> String {
    format!("{function_name}${}", instruction.display_name())
}

fn synthetic_retval_name(function_name: &str) -> String {
...
fn lower_node_transfer(
    ...
    match instruction.get_opcode() {
        ...
        InstructionOpcode::ICmp => {
            ...
            let predicate = match instruction
                .get_icmp_predicate()
                .ok_or_else(|| AdapterError::UnsupportedInstruction(instruction.print()))?
            {
                "==" => Formula::eq(lhs, rhs),
...
fn choose_assert_node(
    site: &crate::llvm_utils::program_graph::AssertSite,
    instruction_nodes: &HashMap<Instruction, CfgNodeId>,
) -> Option<CfgNodeId> {
    instruction_nodes
...
fn assertion_location(site: &crate::llvm_utils::program_graph::AssertSite) -> String {
                    ...
                    if let Some(binding) = env.get(&source) {
                        rewritten.push(TransferEffect::Assign {
                            ...
                            value: AssignValue::Term(Term::select(
                                crate::analysis::formula::Memory::var(&binding.region),
...
pub fn rename_vars_in_memory(
    memory: &crate::analysis::formula::Memory,
    rename: impl Fn(&str) -> String + Copy,
) -> crate::analysis::formula::Memory {
    match memory {
        crate::analysis::formula::Memory::Var(name) => {
            crate::analysis::formula::Memory::var(rename(name))
        }
        crate::analysis::formula::Memory::Store(inner, index, value) => {
            crate::analysis::formula::Memory::store(
...
fn substitute_var_name_with_term_memory(
    ...
    match memory {
        crate::analysis::formula::Memory::Var(memory_name) => {
            crate::analysis::formula::Memory::var(memory_name)
        }
        crate::analysis::formula::Memory::Store(inner, index, value) => {
            crate::analysis::formula::Memory::store(
...
fn memory_contains_var(memory: &crate::analysis::formula::Memory, name: &str) -> bool {
    match memory {
        crate::analysis::formula::Memory::Var(memory_name) => memory_name == name,
        crate::analysis::formula::Memory::Store(inner, index, value) => {
...
mod tests {
    use super::*;
    use crate::analysis::abstract_cfg::TransferEffect;
