...
impl From<crate::analysis::source::SourceLocation> for SourceLocation {
    fn from(value: crate::analysis::source::SourceLocation) -> Self {
...
mod tests {
    ...
    fn wp_composes_in_reverse_order() {
        let transfer = TransferFn::new(vec![
            ...
            TransferEffect::Assign {
                target: Var::int("y"),
                value: AssignValue::Term(Term::var("x", crate::analysis::formula::Sort::Int)),
            },
        ]);
        let post = Formula::eq(
            Term::var("y", crate::analysis::formula::Sort::Int),
            Term::int(0),
        );
        let pre = transfer.wp(&post);
    ...
    fn source_location_from_source_module() {
        let source = crate::analysis::source::SourceLocation::new("f.c", 10, 2);
