...
where
    ...
    M: Fn(SimpleSpan, &'tokens [Spanned<Token<'src>>]) -> I + Clone + 'src,
{
    let ident = select! { Token::FIdent(ident) => ident }.labelled("function name");
    //just(Token::Ident(_))
    ident
...
where
    ...
    M: Fn(SimpleSpan, &'tokens [Spanned<Token<'src>>]) -> I + Clone + 'src,
{
    let ident = select! { Token::FIdent(ident) => ident }.labelled("assertion name");
    //just(Token::Ident(_))
    ident
        //.map_with(|x, e| (x, e.span()))
...
#[cfg(test)]
mod tests {
    use crate::common::expressions::exp::*;
    #[test]
    fn test_num() {
