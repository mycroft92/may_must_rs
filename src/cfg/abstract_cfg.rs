...
// PointerEnv and PointerBinding now live in pointer_analysis::pointer_env.
// Re-export here so callers using `abstract_cfg::{PointerEnv, PointerBinding}`
// continue to compile without change.
pub use crate::pointer_analysis::pointer_env::PointerEnv;
...
impl From<crate::frontend::source::SourceLocation> for SourceLocation {
    fn from(value: crate::frontend::source::SourceLocation) -> Self {
        SourceLocation::new(value.file, value.line, value.column)
    }
}

impl From<SourceLocation> for crate::frontend::source::SourceLocation {
    fn from(value: SourceLocation) -> Self {
        crate::frontend::source::SourceLocation::new(value.file, value.line, value.column)
    }
}
...
mod tests {
    ...
    fn wp_assignment_substitutes_target() {
        ...
        let post = Formula::eq(Term::var("x", crate::formula::Sort::Int), Term::int(3));
    ...
    fn wp_composes_in_reverse_order() {
        let transfer = TransferFn::new(vec![
            ...
            TransferEffect::Assign {
                target: Var::int("y"),
                value: AssignValue::Term(Term::var("x", crate::formula::Sort::Int)),
            },
        ]);
        let post = Formula::eq(Term::var("y", crate::formula::Sort::Int), Term::int(0));
        let pre = transfer.wp(&post);
    ...
    fn sp_assignment_adds_equality() {
        ...
        assert_eq!(
            ...
            Formula::and(
                Formula::bool_var("r"),
                Formula::eq(Term::var("x", crate::formula::Sort::Int), Term::int(8)),
            )
        );
    }
    ...
    fn source_location_from_source_module() {
        let source = crate::frontend::source::SourceLocation::new("f.c", 10, 2);
