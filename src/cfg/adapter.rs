...
use crate::cfg::flat_layout::{eval_flat_addr, FlatLayout};
use crate::cfg::{
    AbstractCfg, AssignValue, CallMemoryEffect, CfgEdgeId, CfgNodeId, PointerEnv, SourceLocation,
    TransferEffect, TransferFn,
};
use crate::formula::{Formula, Memory, Rational, Sort, Term, Var};
use crate::frontend::llvm_wrap::{Instruction, InstructionOpcode, TargetData, TypeKind};
use crate::frontend::program_graph::FunctionGraph;
use crate::pointer_analysis::andersen::AliasResult;
use std::cell::Cell;
...
pub fn compute_return_summary(
    ...
    let relation = crate::formula::alpha_rename::rename_formula_uniform(&entry_state, |name| {
...
fn choose_assert_node(
    site: &crate::frontend::program_graph::AssertSite,
    instruction_nodes: &HashMap<Instruction, CfgNodeId>,
) -> Option<CfgNodeId> {
    instruction_nodes
...
fn choose_assume_node(
    site: &crate::frontend::program_graph::AssumeSite,
    instruction_nodes: &HashMap<Instruction, CfgNodeId>,
) -> Option<CfgNodeId> {
    site.predecessor
        .and_then(|inst| instruction_nodes.get(&inst).copied())
...
fn assertion_location(site: &crate::frontend::program_graph::AssertSite) -> String {
...
fn rename_callee_vars(
    ...
    crate::formula::alpha_rename::rename_formula_uniform(formula, |name| {
...
fn substitute_var_name_with_term_memory(
    memory: &crate::formula::Memory,
    name: &str,
    replacement: &Term,
) -> crate::formula::Memory {
    match memory {
        crate::formula::Memory::Var(memory_name) => crate::formula::Memory::var(memory_name),
        crate::formula::Memory::Store(inner, index, value) => crate::formula::Memory::store(
...
fn memory_contains_var(memory: &crate::formula::Memory, name: &str) -> bool {
    match memory {
        crate::formula::Memory::Var(memory_name) => memory_name == name,
        crate::formula::Memory::Store(inner, index, value) => {
...
mod tests {
    use super::*;
    use crate::cfg::TransferEffect;
    use crate::frontend::llvm_wrap::{initialize_target, Context};
    use crate::frontend::program_graph::generate_program_graph;
